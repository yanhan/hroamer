...
import TestHelpers
       (RowCount, clearDb, deleteTempDirForTest, getTotalRows,
        setupDbForTest)

spec :: Spec

spec = beforeAll (setupDbForTest "Database.InternalSpec") $ afterAll deleteTempDirForTest $ after clearDb $ do
  describe "addFileDetailsToDb" $ do
    it "should add a new row to the database (assuming the (dir, filename) combo does not exist)" $ \pathToDb -> do
      let dir = "/home/edmund"
  ...
  describe "deleteFileFromDb" $ do
    it "should delete the specified dir and filename if they are in the table" $ \pathToDb -> do
      withConnection
        pathToDb
        (\dbconn -> do
    ...
    it "should not change the database if the specified dir and filename are not in the table" $ \pathToDb -> do
      withConnection
        pathToDb
        (\dbconn -> do
